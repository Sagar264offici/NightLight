...
export class FirebaseAuthService {
  ...
  async exchange(idToken: string): Promise<ExchangeResult> {
    ...
    if (!identity.emailVerified || !identity.email) {
      throw new ApiError(403, 'EMAIL_NOT_VERIFIED', 'Verify your email in the link we sent you, then continue.')
    }
    ...
    if (!created) {
      await this.users().updateOne({ _id: user._id }, { $set: { tokenHash, lastSeenAt: now } })
    }
