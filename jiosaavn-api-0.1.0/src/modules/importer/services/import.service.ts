...
export class ImportService {
  ...
  async importPlaylist(rawUrl: string, limit = MAX_IMPORT_TRACKS): Promise<ImportedPlaylist> {
  ...
  private async fetchSpotify(url: URL, wanted: number): Promise<FetchedPlaylist> {
    ...
    const sourceTotal = typeof entity.totalTracks === 'number'
      ? entity.totalTracks
  ...
  private async fetchApple(url: URL, wanted: number): Promise<FetchedPlaylist> {
    const html = await this.httpGet(url.toString())

    // Check for explicit 404 / privacy page.
    if (/<title[^>]*>\s*Not Found\s*<\/title>/i.test(html) ||
    ...
    for (const b of ldBlocks) {
      try {
        ...
        if (items.length) {
          return { source: 'apple-music', name: String(playlist.name ?? 'Apple Music playlist'), items, sourceTotal: items.length }
        }
      } catch {
...
function artistAgrees(a: string, b: string): boolean {
  ...
  const tokensA = new Set(a.split(/\s*[&,/]\s*/).flatMap((s) => s.split(' ')).filter(Boolean))
  const tokensB = new Set(b.split(/\s*[&,/]\s*/).flatMap((s) => s.split(' ')).filter(Boolean))
  let overlap = 0
...
function findYtContinuation(o: unknown): string | null {
  if (Array.isArray(o)) {
    for (const n of o) { const t = findYtContinuation(n); if (t) return t }
    return null
  }
...
function parseTitleFromHtml(html: string): string | null {
  ...
  if (!m) return null
  return m[1].replace(/\s*[-–]\s*YouTube\s*$/i, '').trim().slice(0, 200) || null
}
